  ...
  <div class="content-wrapper">
    <section class="content">
      <div class="container-fluid">
            ...
            <div class="card mt-2 px-1" style="border-top:3px solid tomato;">
              ...
              <div class="row py-2 px-1 border-bottom" style="background: #e0f7e870;">
                <div class="col-11 px-2 d-flex">
                  ...
                  <h5 class="d-flex justify-content-center align-items-center text-center"><?php if($action == 'update'){echo "Update";}else{echo "Add";} ?> Store For Employee</h5> </div>
        ...
        <form role="form" method="post" enctype="multipart/form-data" action="employee_store_add_action.php">
          
          <div class="row px-2 py-3">
            
            <!-- registration page -->
            <div class="col-md-12">
              <h4 style="color:#d9534f;"><b>Store Details:-<?php echo $employee_name; ?></b></h4>
              <hr> </div>
             ...
             <div class="col-lg-12 col-sm-12 col-12 mb-4">
                    ...
                    $data_store = $db->query("SELECT * FROM employee_store WHERE employee_id='$id'");
                    while ($value_user_store = $data_store->fetch_object()) {
                           ...
                           <td>
                              <select class="form-control" name="store_id_update[]" onchange="getStoreUser(this.value)">
                                	<option value="">Select Store</option>
           <?php 
                   $data1store = $db->query("SELECT * FROM `store`");
                    while ($value1store = $data1store->fetch_object()) { ?>
                      <option value="<?php echo $value1store->id; ?>"   style="text-transform: uppercase;" <?php if($value1store->id == $value_user_store->store_id){echo "selected"; } ?>><?php echo  $value1store->store_name; ?></option>
                      <?php } ?>
                                </select> </td>
                            ...
                            <td class="text-center justify-conent-center">
                              <input type="hidden" name="post_id_update[]" value="<?php echo $value_user_store->id; ?>" />
                               <a href="delete_action.php?delete_user_store=delete_user_store&user_store_id=<?php echo $value_user_store->id; ?>&emloyee_id=<?php echo $id; ?>" onClick="return confirm('Are You Sure want to delete ?')" class="btn btn-filter setclose" style="background: #ea5455;" <?php if($action ==='update'){echo 'disabled';} ?>> <i class="fas fa-times text-light"></i> </a>
                           </td>
                          </tr>
                          ...
                          <tr id="TRbodyvariation">
                           ...
                           <td>
                              <select class="form-control" name="store_id[]" onchange="getStoreUser(this.value)">
                                	<option value="">Select Store</option>
           <?php 
                   $data1 = $db->query("SELECT * FROM `store`");
                    while ($value1 = $data1->fetch_object()) { ?>
                      <option value="<?php echo $value1->id; ?>"   style="text-transform: uppercase;"><?php echo  $value1->store_name; ?></option>
                      <?php } ?>
                                </select> </td>
...
function getStoreUser(value){
     var employee_id = $("#employee_id").val();
        $.ajax({
    type: "POST",
    url: 'get_store_user_already.php',
    data: {
      value: value,
    ...
    $("#setloader").removeClass("pageloader");
      if(data.trim()=="yes"){
        alert("User Store Already Exits Please Choose Other!");
