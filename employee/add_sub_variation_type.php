...
if(isset($_GET['post']))
{
	$action = $_GET['action'];
	$id = $_GET['post'];
	$data_emp = $db->query("SELECT * FROM sub_variation_type WHERE id='$id'");
  ...
  <div class="content-wrapper">
    <section class="content">
      <div class="container-fluid">
            ...
            <div class="card mt-2 px-1" style="border-top:3px solid tomato;">
              ...
              <div class="row py-2 px-1 border-bottom" style="background: #e0f7e870;">
                <div class="col-11 px-2 d-flex">
                  ...
                  <h5 class="d-flex justify-content-center align-items-center text-center"><?php if($action == 'update'){echo "Update";}else{echo "Add";} ?> Sub Variation Type</h5> </div>
                  
                  <div class="col-1 px-2 d-flex">
                  <a href="sub_variation_type_list.php" class="mr-2 d-flex justify-content-center align-items-center" style="background-color: tomato;border-radius: 50%; width: 40px; height: 40px; text-align: center;"> <i class="fa fa-arrow-left" style="font-size: 25px; color:#fff;"></i> </a>
                   </div>
        ...
        <form role="form" method="post" enctype="multipart/form-data" action="sub_variation_type_action.php">
           ...
           <?php 
                   $data1 = $db->query("SELECT * FROM `variation` WHERE store_id='$store_id'");
                    while ($value1 = $data1->fetch_object()) { ?>
                      <option value="<?php echo $value1->id; ?>" <?php if($value_emp->variation===$value1->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value1->variation_name; ?></option>
                      <?php } ?>
                                </select> </td>
                                
                                    <td>
                              <select class="form-control" name="sub_variation_name[]" id="sub_variation_id">
                          
                          <option value="">Choose Variation Type</option>      	
           <?php 
                   $data11 = $db->query("SELECT * FROM `sub_variation` WHERE store_id='$store_id'");
                    while ($value11 = $data11->fetch_object()) { ?>
                      <option value="<?php echo $value11->id; ?>" <?php if($value_emp->sub_variation===$value11->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value11->sub_variation_name; ?></option>
                      <?php } ?>
                                </select> </td>
                                
                            <td>
                              <input type="text" name="sub_variation_type[]" value="<?php echo $value_emp->sub_variation_type; ?>" class="form-control" placeholder="Sub Variation Type Name"> </td>
