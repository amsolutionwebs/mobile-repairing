...
case 'publish':
 ...
 $query = $db->query("SELECT * FROM customer_managment WHERE store_id='$store_id' AND name='$name' AND mobile_number='$mobile_number'");
  if($query->num_rows > 0){
 ...
 $db->query("UPDATE `customer_ledger` SET `credit_amount`='$new_old_blance',`blance_amount`='$new_old_blance' WHERE `store_id`='$store_id' AND `employee_id`='$employee_id' AND `customer_id`='$post_id'");
  ...
  case 'delete':
   $post_id = $_GET['post'];
	   ...
	   $db->query("UPDATE customer_managment SET status='disable' WHERE  id = '$post_id'");
