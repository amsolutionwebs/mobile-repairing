...
case 'update':
  ...
  $result_one = $db->query("UPDATE `credit_note` SET `total_credit_note_amount`='$all_total_amount' WHERE `id`='$post_id'");
 ...
 if ($result_one) {
    ...
    {
        $result1 = $db->query("INSERT INTO `credit_note_sort_data`(`credit_note_id`, `invoice_id`, `sort_id`, `hsn_code`, `units`, `quantity`, `rate`, `total_one`) VALUES ('$post_id','$invoice_id','$sort_no[$i]','$hsn_code[$i]','$unit_messure[$i]','$quantity_invoice[$i]','$rate_invoice[$i]','$total_amount_first[$i]')");
