...
case 'publish':
  ...
  $eastimate_amount = mysqli_real_escape_string($db, $_POST['eastimate_amount']);
  $mobile_or_model = mysqli_real_escape_string($db, $_POST['mobile_or_model']);
 ...
 $db->query("INSERT INTO `leads_managment`(`store_id`, `employee_id`, `customer_id`, `leads_no`, `leads_date`, `leads_type`, `device_type`, `eastimate_amount`, `mobile_or_model`, `imei_number`, `product_password`, `address`, `remark1`, `remark2`, `followup_date`, `status`, `date`) VALUES ('$store_id','$user_id','$customer_id','$leads_no','$leads_date','$leads_type','$device_type','$eastimate_amount','$mobile_or_model','$imei_number','$product_password','$address','$remark1','$remark2','$followup_date','$status','$post_date_time')");
  
    //photo upload ends
    
     if ($status==='Converted' && $leads_type==='Repair') {
       
         $db->query("INSERT INTO `repair_managment`(`store_id`, `user_id`, `customer_id`, `eastimate_amount`, `mobile_or_model`, `imei`, `password`, `customer_remark`, `date`) VALUES ('$store_id','$user_id','$customer_id','$imei_number','$eastimate_amount','$mobile_or_model','$product_password','$remark1','$post_date_time')");
  ...
  $post_date_time = date("d-m-Y H:i:s");
   ...
   $update_query = $db->query("UPDATE `leads_managment` SET `customer_id`='$customer_id',`leads_type`='$leads_type',`device_type`='$device_type',`eastimate_amount`='$eastimate_amount',`mobile_or_model`='$mobile_or_model',`imei_number`='$imei_number',`product_password`='$product_password',`address`='$address',`remark1`='$remark1',`remark2`='$remark2',`followup_date`='$followup_date',`status`='$status',`date`='$post_date_time' WHERE `id`='$post_id'");
	
 if ($update_query) {
      
      if ($status==='Converted' && $leads_type==='Repair') {
       
         $db->query("INSERT INTO `repair_managment`(`store_id`, `user_id`, `customer_id`, `imei`, `eastimate_amount`, `mobile_or_model`, `password`, `customer_remark`, `date`) VALUES ('$store_id','$user_id','$customer_id','$eastimate_amount','$mobile_or_model','$imei_number','$product_password','$remark1','$post_date_time')");
