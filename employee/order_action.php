...
for ($i = 0; $i < count($files['name']); $i++) {
	...
	$db->query("INSERT INTO `product_images`(`product_id`,`status`) VALUES ('$product_id','$status')");
 ...
 $lid = $db->insert_id;
    ...
    if ($_FILES["products_image"]["size"][$i] > 0) {
      ...
      $new_products_image = $random_digit ."-". $photo;
        ...
        if ($_FILES["products_image"]["error"][$i] > 0) {
          ...
          $db->query("UPDATE product_images SET product_images = '$new_photo' WHERE id='$lid'");
        }
    ...
    $_SESSION['msg'] = md5('5');
    header("location:product_images_list.php?product_id=$product_id&action=view");
...
$db->query("UPDATE `product_images` SET `status`='$status' WHERE id='$post_id'");
 ...
 //phpto upload
    ...
    $extension = end($temp);
      ...
      if ($_FILES["products_image"]["error"][$k] > 0) {
       ...
       // imagejpeg($p_temp, $imgfile);
		...
		$data = $db->query("SELECT product_images FROM product_images WHERE id = '$post_id'");
        $value = $data->fetch_object();
		if(empty($value->product_images)){
			$db->query("UPDATE product_images SET product_images = '$new_photo' WHERE id = '$post_id'");	
		}else{
  ...
  case 'delete':
   $post_id = $_GET['post'];
   $db->query("DELETE FROM order_managment WHERE  id = '$post_id'");
