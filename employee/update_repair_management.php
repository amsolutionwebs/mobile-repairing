...
if(empty($value_emp->engineer_code)){
...
$value_enge_id = $value_emp->engineer_id;             
$value_employee1 = $value_emp->engineer_code;
}
...
$data_emp = $db->query("SELECT * FROM employee WHERE id='$value_eng->employee_id'");
	$value_employee = $data_emp->fetch_object(); ?>
		...
		<option value="<?php echo $value_eng->employee_id; ?>" <?php if ($value_eng->employee_id==$value_emp->engineer_id) { echo "selected";} ?>>
...
<td width="20">
<input type="text" name="quantity_invoice[]" class="quantity_invoice form-control text-center" id="quantity_invoice_update" placeholder="Quantity" onchange="Calc(this)" value="<?php echo $value_repair_details->quantity; ?>" required> </td>
...
<button type="button" class="btn setclose" style="background: #ea5455;" onclick="TableDelete(this,<?php echo $repair_id; ?>,<?php echo $value_repair_details->id; ?>)"> <i class="fas fa-times text-light"></i> </button>
...
function TableDelete(btndelete,repair_id,product_details_id) {
...
var total3 = $('.total3').val();
				...
				var delete_repair = 'delete_repair';
 						 $.ajax({
        type: "POST",
