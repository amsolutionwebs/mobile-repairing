...
if(isset($_GET['return_id']))
{
  ...
  $data_emp = $db->query("SELECT * FROM return_managment WHERE id='$return_id'");
...
$data = $db->query("SELECT return_number FROM return_managment WHERE store_id='$store_id' ORDER BY id DESC LIMIT 1");
$value = $data->fetch_object();
$recipt_name = $value->return_number;
if(empty($recipt_name))
{
  $freciept_name = "RETURN-"."01";
}else
{
  $explode_val= explode("-",$recipt_name);
  $exe_value =  $explode_val[1]+1;
  $freciept_name = "RETURN-".'0'.$exe_value;
}
...
?>
  ...
  <div class="content-wrapper">
    <section class="content">
      <div class="container-fluid">
            ...
            <div class="card mt-2 px-1" style="border-top:3px solid tomato;">
              ...
              <div class="row py-2 px-1 border-bottom" style="background: #e0f7e870;">
                <div class="col-11 px-2 d-flex">
                  ...
                  <h5 class="d-flex justify-content-center align-items-center text-center">Return</h5> </div>
                  
                  <div class="col-1 px-2 d-flex">
                  <a href="return_management_list.php" class="mr-2 d-flex justify-content-center align-items-center" style="background-color: tomato;border-radius: 50%; width: 40px; height: 40px; text-align: center;"> <i class="fa fa-arrow-left" style="font-size: 25px; color:#fff;"></i> </a>
                   </div>
        ...
        <form role="form" method="post" enctype="multipart/form-data" action="return_management_action.php">
          
          <div class="row px-2 py-3">
            
            <!-- registration page -->
            <div class="col-md-12">
              <h4 style="color:#d9534f;"><b>Customer Details For Return:-</b></h4>
              <hr> </div>
          <div class="col-lg-3 col-sm-6 col-12">
              <div class="form-group">
                <label>Sales Number <b style="color:red;"><sup>*</sup></b></label>
                <select class="form-control select2" name="sales_id" onchange="getSalesDetails11(this.value)">
                  <option value="0">Select sales number</option>
                   <?php 
                  $data_sales = $db->query("SELECT * FROM `sales_managment` WHERE `store_id`='$store_id'");
                    while ($value_sales = $data_sales->fetch_object()) { ?>
                      <option value="<?php echo $value_sales->id; ?>"  <?php if($value_sales->id==$sales_id){echo 'selected';} ?> style="text-transform: uppercase;"><?php echo  $value_sales->sales_number; ?></option>
                      <?php } ?>
                </select>
                 </div>
            </div>
              ...
              <div class="form-group">
                ...
                <input type="text" name="return_number" value="<?php if($action == 'update'){echo $value_emp->purchase_number;}else{echo $freciept_name;} ?>" class="form-control" placeholder="RETURN-123" readonly> </div>
            </div>
           ...
           <div class="col-lg-3 col-sm-6 col-12">
              ...
              <div class="form-group">
                ...
                <input type="text" name="sales_number" value="<?php echo $value_emp->sales_number; ?>" class="form-control" placeholder="Sales-123" readonly> </div>
            </div>
            
            <div class="col-lg-3 col-sm-6 col-12">
              <div class="form-group">
                <label>Sales Date</label>
                <input type="datetime-local" name="sales_date" value="<?php echo $value_emp->sales_date; ?>" class="form-control" readonly required> </div>
            </div>
            
              <div class="col-lg-3 col-sm-6 col-12">
              <div class="form-group">
                <label>Device Type</label>
                  <select class="form-control select2" name="device_type" required>
            ...
            <div class="col-lg-3 col-sm-6 col-12">
              <div class="form-group">
                ...
                <select class="form-control select2" name="warranty_in_days">
                  ...
                  <option value="0_days" <?php if ($value_emp->warranty_in_days=='30_days') { echo "selected";} ?>>0 Days</option>
            ...
            <div class="col-lg-12 col-sm-12 col-12">
              <div class="form-group">
                ...
                <input type="text" name="customer_address" value="<?php echo $value_emp->customer_address; ?>" class="form-control" readonly> </div>
            </div>
  ...
  <!-- add product section -->
 <div class="col-md-12">
              <h4 style="color:#d9534f;"><b>Product Details For Return:-</b></h4>
              <hr> </div>
...
<?php
    ...
    $data_sales_details = $db->query("SELECT * FROM sales_product_details WHERE sales_id='$sales_id'");
    while ($value_sales_details = $data_sales_details->fetch_object()) {
...
<tr id="TRbodyinvoice">
  ...
  <?php 
    ...
    $data_product = $db->query("SELECT * FROM `products` WHERE id='$value_sales_details->product_id'");
    while ($value_products = $data_product->fetch_object()) { ?>
    <option value="<?php echo $value_products->id; ?>" <?php if($value_sales_details->product_id==$value_products->id){echo "selected";} ?>>
 ...
 <td id="mysort" width="150">
   ...
   <?php 
    $data1 = $db->query("SELECT * FROM `variation` WHERE id='$value_sales_details->variation_id'");
    while ($value1 = $data1->fetch_object()) { ?>
    <option value="<?php echo $value1->id; ?>" <?php if($value_sales_details->variation_id===$value1->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value1->variation_name; ?></option>
    <?php } ?>
   </select>
  </td>
<td id="mysort" width="150">
 ...
 <?php 
  $data2 = $db->query("SELECT * FROM `sub_variation` WHERE id='$value_sales_details->sub_variation_id'");
   while ($value2 = $data2->fetch_object()) { ?>
    <option value="<?php echo $value2->id; ?>" <?php if($value_sales_details->sub_variation_id===$value2->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value2->sub_variation_name; ?></option>
 <?php } ?>
</select>
</td>
<td width="50">
<input type="text" name="quantity_invoice[]" class="quantity_invoice form-control text-center" id="quantity_invoice" placeholder="Qty" onchange="Calc(this)" value="<?php echo $value_sales_details->qunatity; ?>" required> </td>
<td width="130">
<input type="text" name="rate_invoice[]" class="rate_invoice form-control text-center" id="rate_invoice" placeholder="Rate" onchange="Calc(this)" value="<?php echo $value_sales_details->rate; ?>"  required> </td>

<td width="130">
<input type="text" name="discount_invoice[]" class="discount_invoice form-control text-center" id="discount_invoice" placeholder="Discount" onchange="Calc(this)" value="<?php echo $value_sales_details->discount; ?>"  required> </td>

<td width="130">
<input type="text" name="gst_invoice[]" class="gst_invoice form-control text-center" id="gst_invoice" placeholder="SERVICE TAX (GST)" onchange="Calc(this)" value="<?php echo $value_sales_details->gst; ?>" required> </td>


<td width="130">
  <input type="text" name="total_amount_first[]" class="total_amount_first form-control text-center" placeholder="Total" id="total_amount_first" value="<?php echo $value_sales_details->amount; ?>" required readonly> </td>
...
<tr>
                  ...
                  <thead style="background: #1b2850;">
                    <tr>
                      ...
                      <th colspan="2"><input type="text" name="all_quantity" class="all_quantity text-center" placeholder="Total Quantity" id="all_quantity" style="border: none;background: #1b2850;color: #fff;" value="<?php echo  $value_emp->all_quantity;?>" ></th>
...
<script>
  ...
  function getSalesDetails11(value) {
      if(value=='0'){
            window.location = 'add_return_management.php?action=add';
        }else{
            window.location = 'add_return_management.php?action=add&sales_id='+value;
        }
  }
