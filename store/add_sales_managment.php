...
if(isset($_GET['post']))
{
  $action = $_GET['action'];
  $id = $_GET['post'];
  $data_emp = $db->query("SELECT * FROM sales_managment WHERE id='$id'");
          ...
          <div class="row px-2 py-3">
           ...
           <div class="col-lg-3 col-sm-6 col-12">
              ...
              <div class="form-group">
                ...
                <input type="text" name="sales_number" value="<?php if($action == 'update'){echo $value_emp->sales_number;}else{echo $freciept_name;} ?>" class="form-control" placeholder="Enq-123" readonly> </div>
            </div>
            
            <div class="col-lg-3 col-sm-6 col-12">
              <div class="form-group">
                <label>Sales Date</label>
                <input type="datetime-local" name="sales_date" value="<?php echo $value_emp->sales_date; ?>" class="form-control" required> </div>
            </div>
            ...
            <div class="col-lg-12 col-sm-12 col-12">
              <div class="form-group">
                ...
                <input type="text" name="customer_address" value="<?php echo $value_emp->customer_address; ?>" class="form-control"> </div>
            </div>
...
<tr id="TRbodyinvoice1">
  ...
  <?php 
    ...
    <option value="<?php echo $value_products->id; ?>" <?php if($value_products->id==$value_sales_details->product_id){echo "selected";} ?>>
 ...
 <td id="mysort" width="180">
   ...
   <?php 
    ...
    <option value="<?php echo $value_variation->id; ?>" <?php if($value_sales_details->variation_id==$value_variation->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value_variation->variation_name; ?></option>
    <?php } ?>
   </select>
  </td>
<td id="mysort" width="180">
 ...
 <?php 
  $data_sub_v = $db->query("SELECT * FROM `sub_variation`");
   while ($value_sub_v = $data_sub_v->fetch_object()) { ?>
    <option value="<?php echo $value_sub_v->id; ?>" <?php if($value_sales_details->sub_variation_id==$value_sub_v->id) { echo "selected"; } ?>  style="text-transform: uppercase;"><?php echo  $value_sub_v->sub_variation_name; ?></option>
 <?php } ?>
</select>
</td>
<td width="80">
<input type="text" name="quantity_invoice_update[]" class="quantity_invoice_update form-control text-center" id="quantity_invoice_update" placeholder="Quantity" onchange="CalcUpdate(this)" value="<?php echo $value_sales_details->qunatity; ?>" required> </td>

<td>
<input type="text" name="rate_invoice_update[]" class="rate_invoice_update form-control text-center" id="rate_invoice_update" placeholder="Rate" onchange="CalcUpdate(this)" value="<?php echo $value_sales_details->rate; ?>"  required> </td>

<td width="130">
<input type="text" name="discount_invoice_update[]" class="discount_invoice_update form-control text-center" id="discount_invoice_update" placeholder="Discount" onchange="CalcUpdate(this)" value="<?php echo $value_sales_details->discount; ?>" required> </td>

<td width="130">
<input type="text" name="gst_invoice_update[]" class="gst_invoice_update form-control text-center" id="gst_invoice_update" placeholder="SERVICE TAX (GST)" onchange="CalcUpdate(this)" value="<?php echo $value_sales_details->gst ?>" required> </td>

<td>
  <input type="text" name="total_amount_first_update[]" class="total_amount_first_update form-control text-center" placeholder="Total" id="total_amount_first_update" value="<?php echo $value_sales_details->amount; ?>" required readonly> </td>

<td class="text-center justify-conent-center">
  <input type="hidden" name="post_id_update[]" value="<?php echo $value_sales_details->id; ?>" />
<a href="delete_action.php?delete_repair=delete_repair&value_repair_id=<?php echo $value_sales_details->id; ?>&post_id=<?php echo $repair_id; ?>" onClick="return confirm('Are You Sure want to delete ?')" class="btn btn-filter setclose" style="background: #ea5455;" <?php if($action ==='update'){echo 'disabled';} ?>> <i class="fas fa-times text-light"></i> </a>
...
function CalcUpdate(value) {
  ...
  var qty_update = document.getElementsByName("quantity_invoice_update[]")[index].value;
  var rate_update = document.getElementsByName("rate_invoice_update[]")[index].value;
  var discount_update = document.getElementsByName("discount_invoice_update[]")[index].value;
  var gst_update = document.getElementsByName("gst_invoice_update[]")[index].value;
  var amt_update = (qty_update) * (rate_update)-(discount_update);

  var final_with_gst_update = (amt_update * gst_update) / 100;
  var final_value_first_update = +(amt_update)+ +(final_with_gst_update);
  document.getElementsByName("total_amount_first_update[]")[index].value = final_value_first_update.toFixed(2);
  total_update();
}

function total_update(value) {
  var amts_update = document.getElementsByName("total_amount_first_update[]");
  var sum_update = 0;
  for(let i = 0; i < amts_update.length; i++) {
    var total_update2 = amts_update[i].value;
    sum_update = +(sum_update) + +(total_update2);
  }
  document.getElementById("total_2").value = sum_update;
 Calc();
}
