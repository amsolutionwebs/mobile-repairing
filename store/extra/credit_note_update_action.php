...
case 'update':
  ...
  $credit_note_date = mysqli_real_escape_string($db, $_POST['credit_note_date']);
  ...
  $result_one = $db->query("UPDATE `credit_note` SET `company_id`='$company_id',`mill_code`='$mill_code',`mill_name`='$mill_name',`sub_mill_code`='$sub_mill_code',`sub_mill_name`='$sub_mill_name',`broker_code`='$broker_code',`dealer_code`='$dealer_code',`dealer_name`='$dealer_name',`credit_note_number`='$credit_note_number',`credit_note_date`='$credit_note_date',`refrence`='$refrence',`invoice_number`='$invoice_number',`doc_encl`='$doc_encl',`transporter`='$transporter',`lr_no`='$lr_no',`lr_date`='$lr_date',`total_credit_note_amount`='$all_total_amount',`remark`='$remark',`date`='$post_date_time' WHERE `id`='$post_id'");
...
$result_two = $db->query("UPDATE `credit_note_payment_details` SET `total_first`='$total_first',`insurance`='$insurance',`total_insurance`='$total_insurance',`cashdiscount`='$cashdiscount',`total_cashdiscount`='$total_cashd',`cgst`='$cgst',`total_cgst`='$total_cgst',`sgst`='$sgst',`total_sgst`='$total_sgst',`igst`='$igst',`total_igst`='$total_igst',`misc_name`='$miscs_name',`misc_value`='$total_miscs',`round_off`='$round_off',`units`='$units',`all_total_amount`='$all_total_amount',`status`='enable' WHERE `credit_note_id`='$post_id'");
 ...
 if ($result_one) {
    ...
    {
        $result1 = $db->query("UPDATE `credit_note_sort_data` SET `credit_note_id`='$post_id',`invoice_id`='$invoice_id',`sort_id`='$sort_no[$i]',`hsn_code`='$hsn_code[$i]',`units`='$unit_messure[$i]',`quantity`='$quantity_invoice[$i]',`rate`='$rate_invoice[$i]',`total_one`='$total_amount_first[$i]' WHERE `id`='$sort_post_id[$i]'");
     ...
     if ($result1) {
      $_SESSION['msg'] = md5('5');
      echo "<script>alert('update success');window.location='credit_note_list.php';</script>";
    }else{
        $_SESSION['msg'] = md5('p_error');
      echo "<script>alert('Updatetion failed');window.location='credit_note_list.php';</script>";
    }
 ...
 $result_without_goods = $db->query("UPDATE `credit_note` SET `company_id`='$company_id',`mill_code`='$mill_code',`mill_name`='$mill_name',`sub_mill_code`='$sub_mill_code',`sub_mill_name`='$sub_mill_name',`broker_code`='$broker_code',`dealer_code`='$dealer_code',`dealer_name`='$dealer_name',`credit_note_number`='$credit_note_number',`credit_note_date`='$credit_note_date',`refrence`='$refrence',`invoice_number`='$invoice_number',`doc_encl`='$doc_encl',`transporter`='$transporter',`lr_no`='$lr_no',`lr_date`='$lr_date',`total_credit_note_amount`='$total_credit_note_amount',`remark`='$remark',`date`='$post_date_time' WHERE `id`='$post_id'");
   ...
   if ($result_without_goods) {
      ...
      echo "<script>alert('Update success');window.location='credit_note_list.php';</script>";
        ...
        $_SESSION['msg'] = md5('p_error');
      echo "<script>alert('Update failed');window.location='credit_note_list.php';</script>";
    }
