...
case 'publish':
  ...
  $all_total_amount = mysqli_real_escape_string($db, $_POST['total3']);
   ...
   $query_wo = $db->query("SELECT * FROM debit_note WHERE debit_note_number='$debit_note_number' AND company_id='$company_id'");
  ...
  $result_one = $db->query("INSERT INTO `debit_note`(`company_id`,`default_mill_code`, `mill_code`, `mill_name`, `sub_mill_code`, `sub_mill_name`, `broker_code`, `dealer_code`, `dealer_name`, `debit_note_number`, `debit_note_date`, `refrence`, `invoice_number`, `doc_encl`, `transporter`, `lr_no`, `lr_date`, `total_debit_amount`, `remark`, `status`, `date`) VALUES ('$company_id','$default_mill_code','$mill_code','$mill_name','$sub_mill_code','$sub_mill_name','$broker_code','$dealer_code','$dealer_name','$debit_note_number','$debit_note_date','$refrence','$invoice_number','$doc_encl','$transporter','$lr_no','$lr_date','$all_total_amount','$remark','unpaid','$post_date_time')");


     $debit_note_id = $db->insert_id;


     $result_two = $db->query("INSERT INTO `debit_note_payment_details`(`debit_note_id`, `total_first`, `insurance`, `total_insurance`, `cashdiscount`, `total_cashdiscount`, `cgst`, `total_cgst`, `sgst`, `total_sgst`, `igst`, `total_igst`, `misc_name`, `misc_value`, `round_off`, `units`, `all_total_amount`, `status`) VALUES ('$debit_note_id','$total_first','$insurance','$total_insurance','$cashdiscount','$total_cashd','$cgst','$total_cgst','$sgst','$total_sgst','$igst','$total_igst','$miscs_name','$total_miscs','$round_off','$units','$all_total_amount','enable')");
 ...
 if ($result_one) {
    ...
    {
        $result1 = $db->query("INSERT INTO `debit_note_sort_data`(`debit_note_id`, `invoice_id`, `sort_id`, `hsn_code`, `units`, `quantity`, `rate`, `total_one`) VALUES ('$debit_note_id','$invoice_id','$sort_no[$i]','$hsn_code[$i]','$unit_messure[$i]','$quantity_invoice[$i]','$rate_invoice[$i]','$total_amount_first[$i]')");
 ...
 $new_old_blance = $old_blance_amount-$total_debit_amount;

   $query_ledger_final2=$db->query("INSERT INTO `ledger`(`company_id`, `default_mill_code`,`dealer_code`,`doc_id`,`invoice_number`, `doc_number`, `doc_date`, `type`, `debit_amount`, `credit_amount`, `blance_amount`,`db_cr`,`status`) VALUES ('$company_id','$default_mill_code','$dealer_code','$debit_note_id','$invoice_number','$debit_note_number','$debit_note_date','debit_note','0.00','$total_debit_amount','$new_old_blance','Db','enable')");
       

   if($query_ledger_final2){
    $_SESSION['msg'] = md5('6');
      echo "<script>alert('success');window.location='debit_note_list.php';</script>";
   }
      
    }else{
        $_SESSION['msg'] = md5('p_error');
      echo "<script>alert('failed');window.location='debit_note_list.php';</script>";
    }
 ...
 $result_without_goods = $db->query("INSERT INTO `debit_note`(`company_id`, `default_mill_code`, `mill_code`, `mill_name`, `sub_mill_code`, `sub_mill_name`, `broker_code`, `dealer_code`, `dealer_name`, `debit_note_number`, `debit_note_date`, `refrence`, `invoice_number`, `doc_encl`, `transporter`, `lr_no`, `lr_date`, `total_debit_amount`, `remark`, `status`, `date`) VALUES ('$company_id','$default_mill_code','$mill_code','$mill_name','$sub_mill_code','$sub_mill_name','$broker_code','$dealer_code','$dealer_name','$debit_note_number','$debit_note_date','$refrence','$invoice_number','$doc_encl','$transporter','$lr_no','$lr_date','$total_debit_amount','$remark','unpaid','$post_date_time')");
 ...
 $new_old_blance = $old_blance_amount-$total_debit_amount;
   ...
   $query_ledger_final=$db->query("INSERT INTO `ledger`(`company_id`, `default_mill_code`,`dealer_code`,`doc_id`,`invoice_number`, `doc_number`, `doc_date`, `type`, `debit_amount`, `credit_amount`, `blance_amount`,`db_cr`,`status`) VALUES ('$company_id','$default_mill_code','$dealer_code','$debit_note_id2','$invoice_number','$debit_note_number','$debit_note_date','debit_note','0.00','$total_debit_amount','$new_old_blance','Db','enable')");
  ...
  case 'delete':
   $post_id = $_GET['post'];
       ...
       $query_wo = $db->query("SELECT refrence FROM debit_note WHERE id='$post_id'");
       $data_debit = $query_wo->fetch_object();
       $refrence =  $data_debit->refrence;

       if($refrence==='Goods_Return'){
       	$db->query("DELETE FROM debit_note WHERE id='$post_id'");
       	$db->query("DELETE FROM debit_note_sort_data WHERE debit_note_id='$post_id'");
        $db->query("DELETE FROM debit_note_payment_details WHERE debit_note_id='$post_id'");
