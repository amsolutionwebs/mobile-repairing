                ...
                <div class="row">
                  <div class="col-1 d-flex">
                    ...
                    <div class="d-flex justify-content-center align-items-center" style="background-color: yellowgreen;border-radius: 50%; width: 50px; height: 50px; text-align: center;">
                      <a href="add_debit_note.php" title="Add Statics Invoice"><i class="fas fa-plus" style="font-size: 30px; color:#fff;"></i></a>
                    </div>
...
?></span>
  ...
  <tbody>
                 <?php
                    ...
                    $data = $db->query("SELECT * FROM debit_note WHERE company_id='$company_login_id'");
                ...
                <tr>
                  ...
                  <td><?php echo $sl; ?></td>
                   <td><?php echo $value->debit_note_number; ?></td>
             ...
             <?php if($value->refrence=='Cash_Discount'){
                  ...
                  <td><?php echo number_format($value->total_debit_amount,2); ?></td>
                   <td><?php echo $value->status; ?></td>
                  <td>
                   
                    
                    <a href='view_debit_note.php?post=<?php echo $value->id; ?>&action=update' title='View Details'><button class='btn btn-xs btn-success mr-2 p-1'><i class='fas fa-eye'></i> </button></a>
                     
                    <?php if($value->refrence=='Goods_Return'){ ?>

                      <a href='add_sort_in_debit_note.php?post=<?php echo $value->id; ?>&invoice_id=<?php echo $value->invoice_number; ?>' title='Update Sort'><button class='btn btn-xs btn-success mr-2 p-1'><i class='fas fa-plus'></i> </button></a>

                   <?php } ?>
                     
                    
                    <a href='update_debit_note.php?post=<?php echo $value->id; ?>&action=update' title='Update'><button class='btn btn-xs btn-warning mr-2 p-1'><i class='fas fa-edit'></i> </button></a>
                              

                               <?php 
                    $delete_query = $db->query("SELECT payment_debit_details.debit_note_number FROM `payment_debit_details` WHERE payment_debit_details.debit_note_number = '$value->id'");
                    $value_delete_query = $delete_query->fetch_object(); 

                    if(empty($value_delete_query->debit_note_number)){ ?>
                      <a href='debit_note_action.php?post=<?php echo $value->id; ?>&action=delete' title='Delete' onClick="return confirm('Are You Sure want to delete ?')" ><button class='btn btn-xs btn-danger p-1'><i class='fas fa-trash'></i> </button></a>
