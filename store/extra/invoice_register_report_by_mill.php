...
$default_mill_code = $_POST['default_mill_code'];

 $query_find_mill = $db->query("SELECT * FROM `mill` WHERE id='$default_mill_code' ");
$value_find_mill = $query_find_mill->fetch_object();
                    
$query_find_sub_mill = $db->query("SELECT * FROM `submill` WHERE id='$default_mill_code'");
...
?>
              ...
              <div class="card-body">
                <div class="row">
                  ...
                  <div class="col-10">
                    ...
                    <h5> Invoice Register By Mill :<?php echo $value_find_mill->mill_name; ?> <?php echo $value_find_sub_mill->mill_name; ?> </h5>
                    <span> From <?php echo $start_date; ?> To <?php echo $end_date ?></span>
                  </div>


                  <div class="col-1 d-flex justify-content-right align-items-right">
                  <div class="d-flex justify-content-center align-items-center" style="background-color: tomato;border-radius: 50%; width: 40px; height: 40px; text-align: center;"> <a href="invoice_register_by_mill.php"><i class="fas fa-arrow-left" style="font-size: 25px; color:#fff;"></i></a> </div>
                </div>
        ...
        <table id="example" class="table table-bordered">
          ...
          <tbody>
             <?php
                    ...
                    $data = $db->query("SELECT * FROM `invoice` WHERE company_id='$company_login_id' AND `default_mill_code`='$default_mill_code' AND `invoice_date` BETWEEN '$start_date' AND '$end_date' ORDER BY id");
                  ...
                  $value_invoice_sort = $data_sort->fetch_object();
                    ...
                    $query_find_dealer = $db->query("SELECT * FROM `dealer` WHERE id='$value_invoice->dealer_code' ");
            ...
            <tr>
              ...
              <td><?php echo $value_find_dealer->dealer_name; ?></td>
  ...
  $final_qty_rate = 0;
  $event_select = $db->query("SELECT id FROM `invoice` WHERE company_id='$company_login_id' AND `default_mill_code`='$default_mill_code' AND `invoice_date` BETWEEN '$start_date' AND '$end_date'");
 ...
 echo $final_qty_rate;
              ...
              <th>
                 <?php
                 $data_ex_amount = $db->query("SELECT sum(total_first) as total_ex_amounte FROM `invoice` WHERE company_id='$company_login_id' AND `default_mill_code`='$default_mill_code' AND `invoice_date` BETWEEN '$start_date' AND '$end_date'");
...
?>
              ...
              <th><?php 
              $query_total_final10 = $db->query("SELECT sum(all_total_amount) as final_value FROM `invoice` WHERE company_id='$company_login_id' AND `default_mill_code`='$default_mill_code' AND `invoice_date` BETWEEN '$start_date' AND '$end_date'");
